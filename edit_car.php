...
if(isset($_GET["car_id"])){
    $car_id=$_GET["car_id"];
    $cars_query=$conn->prepare("SELECT * FROM cars where `id` = :id");
    $cars_query->execute([
        'id' => $car_id
    ]);
...
if(!empty($_FILES)) {
    ...
    $ci_query=$conn->prepare("UPDATE `cars` SET `name` = :name, `model` = :model, `image` = :car_image_name, `description` = :description WHERE `id` = :id");
...
<form method="POST" enctype="multipart/form-data">
  <div class="form-row">
    ...
    <div class="form-group col-md-6">
      ...
      <input type="text" class="form-control" name="model" value="<?php echo $row['model']; ?>" id="model" placeholder="Model">
    </div>
  </div>
  <div class="form-group">
    <label for="description">Description</label>
    <input type="text" class="form-control" name="description" value="<?php echo $row['description']; ?>" id="description" placeholder="Description">
  </div>
  <div class="form-group">
